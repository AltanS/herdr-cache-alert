...
### Added
- `setup` and `client-config` turn on `ui.show_agent_labels_on_pane_borders` in a marked block. Herdr's default is off, so the border badge never showed on a fresh install. A value you set yourself is kept.
